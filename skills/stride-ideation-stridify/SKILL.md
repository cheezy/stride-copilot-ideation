...
Parse arguments in this fixed order — `--goal` first, then the trimmed remainder is `REQUIREMENTS_PATH`:
...
- After flag tokens are consumed, trim the remainder and set `REQUIREMENTS_PATH`. If the remainder is empty, print *"Usage: activate stride-ideation-stridify with `<path-to-requirements.md> [--goal <name|index>]`"* and stop.
