...
- If `--profile` is absent, set `PROFILE` to `lean`. The `lean` profile is byte-for-byte equivalent to upstream v0.3.0 behavior — no new questions, no new sections, no new rubric checks.
